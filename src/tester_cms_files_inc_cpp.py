...
def check_biggest_subnets_uniques(path_to_file,mask):
    ...
    with open(path_to_file) as f:
        ...
        for line in lines:
            ...
            if source_ip != '':
                net = ipaddress.ip_network(source_ip + '/' + str(mask), strict=False)
                if net in nets:
                    if source_ip not in nets[net]:
                        nets[net].append(source_ip)
                else:
                    nets[net] = [source_ip]

    # Printing all element with its count of Occurrence
    # for key,value in nets.items():
    #    print("The Occurrence unique addresses of {0} in the sample list is: {1}".format(key, len(value)))
    
    max_key, max_value = max(nets.items(), key = lambda x: len(set(x[1])))
    print("The Occurrence of unique addresses of {0} in the sample list is: {1}".format(max_key, len(max_value)))
...
def find_p_ipaddress(cms, net):
    ...
    [p.append(int(ip)) for ip in ipaddress.IPv4Network(net) if cms.estimate(int(ip)) > 0]
    ...
    with open(path_to_file) as f:
        ...
        for line in lines:
            ...
            if source_ip != '':
                ...
                source_net = ipaddress.ip_network(source_ip + '/' + str(mask) , strict=False)
...
def run(m=65536, d=6, n=100, mask = 16, step = 100, n_end=-1, iters=10, hash_f='md5', f=None):
    ...
    net = check_biggest_subnets_uniques(f, mask)
    ...
    while True:
        ...
        for i in range(total_iterations):
            ...
            # Store elements in CMS from the CAIDA file
            ...
            p = cms.findpaddress(int(net[0]), int(net[-1]))
